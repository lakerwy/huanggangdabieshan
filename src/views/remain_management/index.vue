  ...
  <div class="remain-management" v-if="reloadValue">
    <div class="page-content">
      <div class="between-flex top">
        <el-button class="add" @click="add" v-if="remain_management_add"
          ><img src="imgs/icon-plus.png" alt="" />新增遗迹</el-button
        >
      ...
      <div class="bottom">
        <div class="between-flex con">
          ...
          <div
            v-if="showBanch"
            class="between-flex opt"
            style="justify-content: flex-end"
          >
            <el-button
              type="default"
              @click="handleTopBtnClick('banch')"
              v-if="remain_management_del"
              ><img src="imgs/icon-banch.png" alt="" />批量操作</el-button
            >
          </div>
          <div v-else class="between-flex opt">
            <el-checkbox v-model="checked" v-if="remain_management_del"
              >全选</el-checkbox
            >
            <el-button
              type="default"
          ...
          <div class="list-item" v-for="(item, index) in lists" :key="item.id">
            <div class="info">
              <img
                ...
                class="pic"
                :src="item.img || detailImg"
                alt=""
                @click="handleDetails(item, remain_management_view)"
              />
              <p class="r-title">{{ item.title }}</p>
              <p class="label">
                <el-button round v-for="t in item.type" :key="t">{{
                  t
                }}</el-button>
              </p>
            </div>
            <div class="operations">
              ...
              <div class="btns">
                <img
                  ...
                  title="定位"
                  @click="handleClick(item.id, 'location')"
                />
                <img
                  v-if="remain_management_edit"
                  src="imgs/icon-edit.png"
                  alt="编辑"
                  @click="handleClick(item.id, 'edit')"
                />
                <img
                  v-if="remain_management_del"
                  src="imgs/icon-del.png"
                  alt="删除"
                  @click="handleClick(item.id, 'del')"
                />
              </div>
            </div>
            <div
...
import MapCon from './index/mapCon'
import * as map from "./index/map";
...
export default defineComponent({
  ...
  data() {
    return {
      remain_management_add: false,
      remain_management_view: false,
      remain_management_edit: false,
      remain_management_del: false
    }
  },
  computed: {
    ...mapGetters([
  ...
  created() {
    this.remain_management_add = this.permissions['remain_management_add']
    this.remain_management_view = this.permissions['remain_management_view']
    this.remain_management_edit = this.permissions['remain_management_edit']
    this.remain_management_del = this.permissions['remain_management_del']
  },
  setup() {
    ...
    // 新增 查询
    const add = () => {
      router.push({ name: 'remain_management/add' })
    }
    ...
    const btns = [
      { type: 'location', icon: 'imgs/icon-location.png', title: '定位' },
      { type: 'edit', icon: 'imgs/icon-edit.png', title: '编辑' },
      { type: 'del', icon: 'imgs/icon-del.png', title: '删除' },
    ]
    ...
    const handleClick = (id, type) => {
      if (type == 'del') {
        ...
        // params.type = 'edit'
        router.push({
          name: 'remain_management/edit', query: {
            type: 'edit',
            id: id
          }
        })
    ...
    //查看详情页
    const handleDetails = (item, flag) => {
      if (flag) {
        router.push({
          name: 'remain_management/detail', query: {
            type: 'details',
            id: item.id
          }
        })
    ...
    const searchAllData = () => {
      currentPage.value = 1;
      getdRelicsPageInfo(null, 1, queryValue.value)
    }
    ...
    const getdRelicsPageInfo = async (level = null, current = 1, name = '') => {
      checked.value = false
      showBanch.value = true;
      isshowMark.value = false;
    ...
    const handleCurrentChange = (val) => {
      getdRelicsPageInfo(labelLeve.value, val, queryValue.value)
    }
    return {
      detailImg,
      reloadValue,
