  ...
  <div>
    ...
    >
      <el-form ref="form" :model="form" label-width="80px" :rules="rules">
        ...
        <el-form-item label="标本排序" prop="order">
          <el-input-number
            v-model="form.order"
            :min="1"
            :max="10000"
        ...
        <el-form-item label="标本类别" prop="parentId">
          <el-select v-model="form.parentId" placeholder="请选择类别">
            <el-option
              :label="item.name"
              :value="item.id"
        ...
        <el-form-item label="标本详情" prop="info">
...
export default {
  ...
  data() {
    return {
      ...
      rules: {
        ...
        info: [
          { required: false, message: '请输入标本详情', trigger: 'blur' }
        ],
        order: [
          { required: true, message: '请输入标本序号', trigger: 'change' }
        ],
        parentId: [
  ...
  watch: {
    dataInfo(val) {
      ...
      this.dynamicTags = val.label?val.label.split(','):[];
      val.pics.filter(obj => {
    ...
    confirm() {
      ...
      that.$refs['form'].validate((valid) => {
        if (valid) {
          ...
          formData.append("label", that.dynamicTags);
          formData.append("type", 2);
          if (this.dataType == 'add') {
            axios({
              url: window.globalUrl.HOME_API + '/datamanager/sample/addSample',
              method: 'POST', //请求方式
              ...
              if (res.success == true) {
                ...
                that.filterRoom(this.form.parentId);
            ...
            axios({
              url: window.globalUrl.HOME_API + '/datamanager/sample/upSample',
              method: 'POST', //请求方式
              headers: { 'content-type': 'application/x-www-form-urlencoded' },
              data: formData,
            }).then(res => {
