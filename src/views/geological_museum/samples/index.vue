      ...
      <div class="content">
        <div class="top">
          <div class="top_left">
            <span class="back" @click="goTo('geological_museum')"
              >返回上一级</span
            >
            <span class="ztgl">馆藏标本</span>
            <span class="add" @click="addExhibition" v-if="samples_add"
              ><i class="el-icon-plus"></i> 新增类别</span
            >
          </div>
          <div class="top_right">
            <el-input
        ...
        <div class="control">
          <div class="tabControl">
            <span @click="delExh()" class="del" v-if="samples_del"
              ><i class="el-icon-circle-close"></i> 删除类别</span
            >
            <span @click="editExh()" v-if="samples_edit"
              ><i class="el-icon-edit"></i> 编辑类别</span
            >
          </div>
          <div
          ...
          <div v-else class="opt">
            <el-checkbox v-model="checked" v-if="samples_del">全选</el-checkbox>
            <el-button
              type="default"
              size="medium"
              v-if="samples_del"
        ...
        <ul class="collections">
          <li
            ...
            v-if="tabArr.length > 0 && samples_add"
          >
...
import { getSampleTypes, getSampleInfo, getSamplesArr, delSamples, delSample } from "_api/museum";
import addSample from "./addSample";
import addSampleType from "./addSampleType.vue";
...
export default {
  ...
  methods: {
    ...
    async batchDelRelicsCard(arr) {
      ...
      let res = await delSamples(params);
      if (res.success) {
        this.showBanch = true;
        this.isshowMark = false;
        this.getCollections(this.params);
    ...
    delExh() {
      this.$confirm('确定删除类别：' + this.tabObj.name + '？', '提示', {
        confirmButtonText: '确定',
        cancelButtonText: '取消',
        type: 'error'
      }).then(() => {
        delSample({ id: this.tabObj.id }).then(res => {
          if (res.success) {
            this.$message({
              type: 'success',
              message: '删除成功!'
    ...
    delCol(item) {
      this.$confirm('确定删除标本：' + item.name + '？', '提示', {
        confirmButtonText: '确定',
        cancelButtonText: '取消',
        type: 'error'
      }).then(() => {
        delSample({ id: item.id }).then(res => {
          if (res.success) {
            this.$message({
              type: 'success',
              message: '删除成功!'
    ...
    dealCollection(type, item) {
      if (!this.samples_view && type == 'view') {
        this.$message({
          type: 'error',
      ...
      if (type !== 'add') {
        getSampleInfo({ id: item.id }).then(res => {
          if (res.success) {
      ...
      getSampleTypes({}).then((response) => {
        if (response.body) {
          if (response.body.length > 0) {
            ...
            this.tabObj = response.body[0];
            this.params.id = response.body[0].value;
            this.getCollections(this.params);
          } else {
            this.tabArr = [];
            this.collectionArr = [];
    ...
    getCollections(params) {
      this.collectionArr = [];
      getSamplesArr(params).then((response) => {
        if (response.success) {
