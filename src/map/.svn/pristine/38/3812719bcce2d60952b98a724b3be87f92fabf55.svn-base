 ...
 * @ Modified time: 2022-01-18 18:05:18
...
import { getStyles } from "../layer/vectorLayer";
...
class DrawClass {
   ...
   * @param {Style| Array<Style> | function} style
   */
  constructor({
    map,
    layer,
    ...
    // sketchFeature 样式
    this.style = style ? getStyles(style) : null;
    // 当前绘制回合类的要素集
  ...
  addDrawInteraction() {
    ...
    if (params && params.type) {
      ...
      if (this.style) {
        opts.style = this.style;
      }
      this.draw_ = new Draw(opts);
