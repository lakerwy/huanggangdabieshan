...
class DxXYZLayer extends TileLayer {
  ...
  constructor(options) {
    ...
    let tileUrlFunction = null;
    if(url.indexOf("reverseY")!= -1){
      tileUrlFunction = function(tileCoord){
        let z = tileCoord[0];
        let x= tileCoord[1];
        let y = Math.pow(2,z)-1- tileCoord[2];
        return options.url.replace("{z}", z,toString()).replace("{x}", x,toString()).replace("{reverseY}", y,toString());
      }
      url = null;
    }
